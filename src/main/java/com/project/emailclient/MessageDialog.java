...
public class MessageDialog extends JDialog {
    ...
    // Constructor for dialog — delegates to the defaultFrom variant with no default.
    public MessageDialog(Frame parent, int type, Message message)
    throws Exception {
        this(parent, type, message, null);
    }

    // Constructor for dialog with an optional pre-filled From address.
    public MessageDialog(Frame parent, int type, Message message, String defaultFrom)
    throws Exception {
    ...
    /* Set dialog title and get message's "to", "subject"
       and "content" values based on message type. */
        ...
        if (defaultFrom != null && !defaultFrom.isBlank()) {
            fromTextField.setText(defaultFrom);
        }
        constraints = new GridBagConstraints();
        ...
        showFileDialogButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                final int returnVal = fileDialog.showOpenDialog(buttonsPanel);
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    file = fileDialog.getSelectedFile();
                }
            }
        });
        buttonsPanel.add(showFileDialogButton);
   ...
   /* MessageDialog(EmailClient aThis, int type, Message message) {
    ...
    // Show dialog and return true if the user clicked Send.
